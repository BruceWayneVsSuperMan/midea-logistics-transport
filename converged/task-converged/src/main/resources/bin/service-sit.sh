...
APP_NAME=task-converged-2.0.0.jar
ENV=sit
BAK_NUM=3
...
start() {
   ...
   if [ $psid -ne 0 ]; then
      ...
      cd $APP_HOME
      nohup $JAVA_HOME/bin/java -server -Xms512m -Xmx512m -Djava.security.egd=file:/dev/./urandom -Djava.awt.headless=true -jar $APP_HOME/$APP_NAME --spring.profiles.active=$ENV >/dev/null 2>&1 &
